...
from flexround_paper_gw import FlexRoundREM
...
def main():
    p = argparse.ArgumentParser("FlexRound (paper-exact, GROUP-WISE) quantization")
    ...
    p.add_argument('--group-size', type=int, default=128,
                   help='group-wise weight quant group size along input dim; '
                        'set >= max in_features for per-channel.')
    ...
    out = os.path.join(args.output_dir, f"flexround_paper_gw_{args.bits}bit")
